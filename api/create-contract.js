export default async function handler(req, res) {
  ...
  // Deutsches Format dd.mm.yyyy direkt zurückgeben
    ...
    // Gesamtpreis aus Einzelpositionen neu berechnen
    const recalcExtras =
      ...
      (data.unlimited_km && data.unlimited_km !== 'false' ? parseFloat(data.unlimited_km_cost || 0) : 0) +
      (data.saturday_handover && data.saturday_handover !== 'false' ? parseFloat(data.saturday_handover_cost || 0) : 0) +
    ...
    const displayTotal = parseFloat(data.daily_rate || 0) * parseFloat(data.rental_days || 0) + recalcExtras - effectiveDiscount
    const calcDownPayment = (displayTotal * 0.3).toFixed(2)
    const calcFinalPayment = (displayTotal * 0.7).toFixed(2)
...
<table>
  ...
  ${hasDiscount ? `<tr><td style="color:green;">${data.discount_code && data.discount_code !== 'null' ? 'Rabattcode (' + data.discount_code + ')' : 'Rabatt'}</td><td style="color:green;">-${fmtPrice(data.discount_amount)} EUR</td></tr>` : ''}
  <tr style="font-weight:bold"><td>Gesamtbetrag</td><td>${fmtPrice(displayTotal)} EUR</td></tr>
</table>
<p>7. Zahlungsplan:</p>
<ul>
  <li>Anzahlung: <strong>${fmtPrice(calcDownPayment)} EUR</strong> bis zum <strong>${downPaymentDueDisplay}</strong></li>
  <li>Restzahlung: <strong>${fmtPrice(calcFinalPayment)} EUR</strong> bis zum <strong>${finalPaymentDueDisplay}</strong></li>
</ul>
